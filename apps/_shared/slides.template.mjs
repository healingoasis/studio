// Slide renderers, shared by both campaigns. __URL__ and __FOOT__ are replaced
...
const foot = (tone, extra = '') => `
  <div style="position:absolute;left:56px;right:56px;bottom:46px;display:flex;align-items:center;justify-content:space-between;gap:24px">
    <div style="font-size:${T.label}px;font-weight:800;color:${tone === 'light' ? 'rgba(255,255,255,0.86)' : C.maroon};letter-spacing:0.01em">__URL__</div>
    ${extra}
  </div>`
...
export const render = (sl, th, i, total) => {
          ...
          <div>
            <div style="width:100%;height:326px;overflow:hidden;border-radius:5px;background:${C.deep}">
              ${f === '11_Angela_Polmateer'
                ...
                : `<img src="../../img/heads/${f}.jpg" alt="" style="width:100%;height:100%;object-fit:cover;object-position:50% 26%;display:block">`}
            </div>
  ...
  if (sl.kind === 'cta') return `
    ${back}${top}
    <div style="position:absolute;left:56px;right:56px;top:50%;transform:translateY(-50%)">
      <div style="font-size:${T.label}px;font-weight:800;letter-spacing:0.24em;text-transform:uppercase;color:${acc}">${sl.kicker || '__CTA__'}</div>
      ...
      ${sl.sub ? `<p style="margin-top:30px;font-size:${T.lead}px;line-height:1.28;font-weight:600;color:${subC};max-width:900px">${sl.sub}</p>` : ''}
      <div style="margin-top:48px;display:inline-block;background:${onPhoto ? '#ffffff' : th.accent};border-radius:6px;padding:26px 40px;font-size:${T.body}px;font-weight:800;color:${onPhoto ? C.maroon : (th.ctaInk || '#fff')}">__URL__</div>
    </div>
    <div style="position:absolute;left:56px;right:56px;bottom:46px;font-size:${T.label}px;font-weight:800;letter-spacing:0.10em;text-transform:uppercase;color:${subC}">${sl.foot || '__FOOT__'}</div>`
