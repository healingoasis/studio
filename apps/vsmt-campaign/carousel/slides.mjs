// Slide renderers, shared by both campaigns. healingoasis.edu — VSMT Program and 2027 Spring and Fall classes — both open are replaced
...
const foot = (tone, extra = '') => `
  <div style="position:absolute;left:56px;right:56px;bottom:46px;display:flex;align-items:center;justify-content:space-between;gap:24px">
    <div style="font-size:${T.label}px;font-weight:800;color:${tone === 'light' ? 'rgba(255,255,255,0.86)' : C.maroon};letter-spacing:0.01em">healingoasis.edu — VSMT Program</div>
    ${extra}
  </div>`
...
export const render = (sl, th, i, total) => {
    ...
    <div style="position:absolute;left:56px;right:56px;top:186px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px 24px">
        ${sl.people.map(([f, name, note]) => `
          <div>
            <div style="width:100%;height:292px;overflow:hidden;border-radius:5px;background:${C.deep}">
              ${false
  ...
  if (sl.kind === 'cta') return `
    ...
    <div style="position:absolute;left:56px;right:56px;top:50%;transform:translateY(-50%)">
      ...
      ${sl.sub ? `<p style="margin-top:30px;font-size:${T.lead}px;line-height:1.28;font-weight:600;color:${subC};max-width:900px">${sl.sub}</p>` : ''}
      <div style="margin-top:48px;display:inline-block;background:${onPhoto ? '#ffffff' : th.accent};border-radius:6px;padding:26px 40px;font-size:${T.body}px;font-weight:800;color:${onPhoto ? C.maroon : (th.ctaInk || '#fff')}">healingoasis.edu — VSMT Program</div>
    </div>
    <div style="position:absolute;left:56px;right:56px;bottom:46px;font-size:${T.label}px;font-weight:800;letter-spacing:0.10em;text-transform:uppercase;color:${subC}">${sl.foot || '2027 Spring and Fall classes — both open'}</div>`
