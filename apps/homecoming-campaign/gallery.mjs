...
const jpgs = readdirSync(new URL('./jpg/', import.meta.url)).filter(f => f.endsWith('.jpg')).sort()

const cards = []
for (const [i, f] of files.entries()) {
  const { meta } = await import(new URL(f, dir))
  const b64 = readFileSync(new URL(`./jpg/${jpgs[i]}`, import.meta.url)).toString('base64')
  cards.push(`
  <figure class="card">
    <img src="data:image/jpeg;base64,${b64}" alt="${meta.angle}">
    <figcaption>
...
writeFileSync(new URL('./review.html', import.meta.url), `<title>Homecoming Campaign</title>
<style>
  :root { --bg:#faf6ef; --ink:#2a1512; --muted:#6b5550; --line:rgba(92,1,1,0.16); --maroon:#5c0101; --ember:#c9502a; }
  :root:not([data-theme="light"]) { }
  @media (prefers-color-scheme: dark) {
    :root:not([data-theme="light"]) { --bg:#1b0f0d; --ink:#f5ecdf; --muted:#b49f98; --line:rgba(245,236,223,0.18); --maroon:#e8d9c8; --ember:#e0714a; }
  }
  :root[data-theme="dark"] { --bg:#1b0f0d; --ink:#f5ecdf; --muted:#b49f98; --line:rgba(245,236,223,0.18); --maroon:#e8d9c8; --ember:#e0714a; }
...
<div class="wrap">
  <header>
    <h1>2026 Homecoming Conference<br>Thirty-day campaign</h1>
    <p class="sub">Twelve posts, 25 August to 27 September. Every post is its own design &mdash; no layout repeats.
    Images are 1080&times;1350 for Facebook; the caption for each one sits in the schedule file alongside them in Drive.
    Every fact was checked against healingoasis.edu on 24 August 2026.</p>
  </header>
